...
- [x] T001 [P] Create intent schema definitions (`MessageIntent` enum and `IntentClassificationResult`) in `backend/app/schemas/intent.py`
- [x] T002 [P] Extend `MessageBase` and `Message` models with `metadata_json` mapped to `metadata` JSON column in `backend/app/models/conversation.py`
...
- [x] T003 Update `ConversationRepository.append_message` to accept and persist `metadata: Optional[dict] = None` in `backend/app/repos/conversation_repo.py`
- [x] T004 [P] Add active document title retrieval helper `DocumentRepo.get_tenant_document_titles` in `backend/app/repos/document_repo.py`
- [x] T005 Create baseline intent routing test harness in `backend/tests/test_chat_intent.py`
...
- [x] T006 [P] [US1] Implement Tier 1 heuristic pattern evaluator for greetings, gratitude, and farewells in `backend/app/services/intent_service.py`
- [x] T007 [US1] Implement greeting and courtesy response generators contextualized with organization business name in `backend/app/services/intent_service.py`
- [x] T008 [US1] Integrate Tier 1 fast-path routing into `stream_chat` in `backend/app/services/chat_service.py` to bypass Qdrant retrieval for standalone pleasantries
- [x] T009 [US1] Persist `{"intent": "GREETING"}` metadata on the assistant message record in `backend/app/services/chat_service.py`
- [x] T010 [P] [US1] Add unit and integration tests for greeting, gratitude, and farewell detection in `backend/tests/test_chat_intent.py`
...
- [x] T011 [US2] Implement compound inquiry disambiguation in `backend/app/services/intent_service.py` to distinguish pure greetings from compound queries
- [x] T012 [US2] Update `build_system_prompt` in `backend/app/services/chat_service.py` to instruct the LLM to acknowledge greetings naturally while grounding facts strictly in retrieved chunks
- [x] T013 [P] [US2] Add unit tests verifying hybrid greeting + question queries proceed to semantic search in `backend/tests/test_chat_intent.py`
...
- [x] T014 [US3] Implement out-of-scope query detector and deflection formatter in `backend/app/services/intent_service.py`
- [x] T015 [US3] Integrate document title topic suggestions into ungrounded retrieval fallback in `backend/app/services/chat_service.py`
- [x] T016 [P] [US3] Add unit tests for out-of-scope deflection and document topic suggestion in `backend/tests/test_chat_intent.py`
...
- [x] T017 [US4] Implement explicit escalation keywords and frustration pattern detection in `backend/app/services/intent_service.py`
- [x] T018 [US4] Integrate immediate de-escalation response and `escalate_suggestion` SSE event dispatch in `backend/app/services/chat_service.py`
- [x] T019 [P] [US4] Add unit tests for explicit human escalation routing in `backend/tests/test_chat_intent.py`
...
- [x] T020 [US5] Implement identity and capability query detection in `backend/app/services/intent_service.py`
- [x] T021 [US5] Implement identity response generator in `backend/app/services/intent_service.py` and integrate with `backend/app/services/chat_service.py`
- [x] T022 [P] [US5] Add unit tests for bot identity queries in `backend/tests/test_chat_intent.py`
...
- [x] T023 [P] Add intent badge rendering in the Owner Conversations Inbox UI in `frontend/components/conversations/conversation-transcript.tsx`
- [x] T024 Verify all scenarios in `specs/014-chat-intent-handling/quickstart.md`
- [x] T025 Confirm all tasks in tasks.md are marked `[X]`
