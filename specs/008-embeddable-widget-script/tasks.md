...
- [x] T001 Create standalone test storefront fixture in `frontend/public/test-widget.html` with host CSS resets
- [x] T002 Initialize base script structure and IIFE wrapper in `frontend/public/widget.js`
...
- [x] T003 [P] Define `ChatEscalateRequest` and `ChatEscalateResponse` schemas in `backend/app/schemas/chat.py`
- [x] T004 Implement `escalate_conversation` in `backend/app/services/chat_service.py` to flag `Conversation.is_escalated = True` and record audit message
- [x] T005 Implement `POST /api/v1/widget/chat/escalate` endpoint in `backend/app/routers/widget.py`
- [x] T006 [P] Add contract tests for escalation in `backend/tests/test_widget_escalate.py`
...
- [x] T007 [US1] Implement script tag attribute extraction (`data-widget-key`, `data-api-base`) in `frontend/public/widget.js`
- [x] T008 [US1] Implement public config retrieval (`GET /api/v1/widget/config`) with silent graceful failure on API/network errors in `frontend/public/widget.js`
- [x] T009 [US1] Implement client-side domain authorization check (`allowed_origins` vs `window.location.hostname`) in `frontend/public/widget.js`
- [x] T010 [US1] Implement Shadow DOM creation (`attachShadow({ mode: "open" })`) and host container mount in `frontend/public/widget.js`
- [x] T011 [US1] Implement encapsulated CSS styles with dynamic tokens (`--rd-primary`), mobile full-screen media queries (< 640px), and launcher animations in `frontend/public/widget.js`
- [x] T012 [US1] Implement floating launcher button with placement support (`bottom-right`, `bottom-left`), click toggle handler, and SVG icons in `frontend/public/widget.js`
- [x] T013 [US1] Implement chat window markup (sticky header, bot display name, status dot, close button, and welcome message bubble) in `frontend/public/widget.js`
...
- [x] T014 [P] [US2] Enhance `backend/app/services/chat_service.py` to yield `event: citation` SSE payloads containing retrieved document titles and ids
- [x] T015 [US2] Implement `localStorage` session manager (`resolvdesk_session_${widgetKey}`) with 24-hour sliding TTL and rehydration in `frontend/public/widget.js`
- [x] T016 [US2] Implement chat input form with 1,000-character validation, debounce protection, and auto-growing/auto-scroll behavior in `frontend/public/widget.js`
- [x] T017 [US2] Implement SSE streaming consumer (`POST /api/v1/widget/chat`) using native `ReadableStream` reader and `TextDecoder` in `frontend/public/widget.js`
- [x] T018 [US2] Implement progressive token bubble rendering with animated typing indicator and auto-scroll in `frontend/public/widget.js`
- [x] T019 [US2] Implement citation badge UI rendering (`[1] Document Title`) below completed assistant messages in `frontend/public/widget.js`
- [x] T020 [US2] Implement network error retry prompt when streaming is interrupted in `frontend/public/widget.js`
...
- [x] T021 [P] [US3] Update `backend/app/services/chat_service.py` to yield `event: escalate_suggestion` when similarity score is below threshold or fallback triggered
- [x] T022 [US3] Implement reactive inline "Escalate to Human Agent" button rendering upon `escalate_suggestion` or fallback in `frontend/public/widget.js`
- [x] T023 [US3] Implement expandable inline escalation form (email input with validation and optional note) in `frontend/public/widget.js`
- [x] T024 [US3] Implement escalation API submission (`POST /api/v1/widget/chat/escalate`) with loading state and error handling in `frontend/public/widget.js`
- [x] T025 [US3] Implement inline ticket confirmation card rendering (displaying ticket ID and email notice) while keeping the chat input active in `frontend/public/widget.js`
...
- [x] T026 [P] Verify bundle size of `frontend/public/widget.js` is under 40 KB gzipped
- [x] T027 Run full backend test suite (`uv run pytest`) ensuring all existing and new tests pass
- [x] T028 Run frontend production build (`npm run build`) ensuring zero compilation or type errors
- [x] T029 Execute full quickstart validation walkthrough in `specs/008-embeddable-widget-script/quickstart.md` using `frontend/public/test-widget.html`
