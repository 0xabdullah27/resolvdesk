...
export function DashboardOverviewView({ owner }: DashboardOverviewViewProps) {
  ...
  React.useEffect(() => {
    ...
    if (trends.status === "idle") loadTrends(30);
    if (knowledgeGaps.status === "idle") loadKnowledgeGaps(5, 30);
    if (topQuestions.status === "idle") loadTopQuestions(5, 30);
    if (documents.status === "idle") loadDocuments();
  }, [
    overview.status,
    trends.status,
    knowledgeGaps.status,
    topQuestions.status,
    documents.status,
    loadOverview,
    loadTrends,
  ...
  const docCount = documents.data?.total || 0;
  ...
  const currentTrends = trends.data || {
    range_days: 30,
    points: [],
  };

  const currentGaps = knowledgeGaps.data?.items || [];
  const currentTopQuestions = topQuestions.data?.items || [];
  ...
  return (
    <div className="space-y-8">
      ...
      <AnalyticsKpiCards overview={currentOverview} />

      {/* 3. Interactive Volume Trends Chart */}
      <VolumeTrendsChart initialData={currentTrends} />

      {/* 4. Actionable Insights Grid: Knowledge Gaps & Top Questions */}
      <div className="grid gap-6 md:grid-cols-2">
        <KnowledgeGapsCard items={currentGaps} />
        <TopQuestionsCard items={currentTopQuestions} />
      </div>
      ...
      <Card className="border-border/70 bg-card">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            {docCount > 0 && currentOverview.total_conversations > 0 ? (
