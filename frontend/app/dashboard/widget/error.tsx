...
export default function WidgetError({
  ...
  React.useEffect(() => {
    console.error("Widget customizer error boundary caught:", error);
  }, [error]);
      ...
      <Card className="w-full max-w-md border-destructive/30 bg-card text-card-foreground shadow-sm">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="text-lg font-semibold text-foreground">
            Failed to Load Widget Customizer
          </CardTitle>
          <CardDescription className="text-xs text-muted-foreground">
            {error.message || "An unexpected error occurred while fetching your organization's widget configuration."}
          </CardDescription>
        </CardHeader>
